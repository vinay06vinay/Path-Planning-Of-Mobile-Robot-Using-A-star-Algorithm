...
def astar(obstacle_matrix,start_x,start_y,start_theta,goal_x,goal_y,goal_theta,step):
    ...
    while(len(open_list)>0):
        ...
        child_parent_dict[(int(current_node[0]),int(current_node[1]))] = (int(parent_node[0]),int(parent_node[1])) 
                    ...
                    if(check_duplicate_flag):
                        for i in open_list:
                            if (new_node == i[3]):
                                ...
                                # print(cost,current_total_cost)
                                if(cost < current_total_cost):
                                    print(i)
                                    i[0] = cost
                                    i[1] = current_cost2c
                                    i[2] = cn[2]
...
'''
def visualise(canvas,optimal_list,explored_dict):
    fourcc = cv2.VideoWriter_fourcc(*'XVID')    # Creating video writer to generate a video.
    out = cv2.VideoWriter('A-Star.avi',fourcc,100,(1920,1080))
    for key,values in explored_dict.items():
        for i in values:
            pt1 = key[:2]
            pt2 = i[:2]
            cv2.arrowedLine(canvas, pt1, pt2, (255,255,255), 1) 
...
def main():
    ...
    if(goal_reached == False):
        print("No Goal Found")
        return
    print(len(child_parent_dict),len(explored_dict),len(closed_list))
    ...
    visualise(canvas,optimal_list,explored_dict)
